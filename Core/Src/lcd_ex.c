 ...
 * @brief       LCD driver IC register initialization
...

{
    ...
    lcd_wr_data((SSD_HOR_RESOLUTION - 1) >> 8); /* LCD姘村钩 */

    lcd_wr_data(SSD_HOR_RESOLUTION - 1);

    lcd_wr_data((SSD_VER_RESOLUTION - 1) >> 8); /* LCD风洿 */
    ...
    lcd_wr_regno(0xF0); /* SSD1963稢PU帴鍖℃涓�16bit */

    lcd_wr_data(0x03);  /* 16-bit(565 format) data for 16bpp */



    lcd_wr_regno(0x29); /* 风ず */

    /* PWM  閫氶鍗犻绉告瘮鍙�纰夋 */

    lcd_wr_regno(0xD0); /* 熺殕璁规峰钩稤BC */
    ...
    lcd_wr_data(0x05);  /* 1PWM棰戦 */

    lcd_wr_data(0xFE);  /* 2PWM鍗犻绉告唻� */
    ...
    lcd_wr_data(0x03);  /* 2稩O熺煫绛规 */

    lcd_wr_data(0x01);  /* GPIO浣块稩O */
